...
function validateModelName(model) {
  ...
  if (!value) return 'gpt-5.6-luna';
  if (/^sk[-_]/i.test(value) || /^sk-proj-/i.test(value)) {
    throw new Error('OPENAI_MODEL is misconfigured. Set it to a model id such as gpt-5.6-luna, not an API key.');
  }
  return value;
}
...
async function callOpenAI({ apiKey, model, system, user }) {
  ...
  if (!response.ok) {
    ...
    throw new Error(sanitizeOpenAIError(message));
  }
